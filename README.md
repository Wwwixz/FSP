...
может не доверять. На современных JDK (проверено на Adoptium 25, 12.09.2026)
строгая проверка проходит без дополнительных настроек; если появится ошибка
PKIX/handshake, для локальной разработки можно включить dev-флаг
`AI_GIGACHAT_INSECURE_SSL=true` (TrustAll) — **ни в коем случае не включайте
его в продакшене**. Если сеть блокирует `https://api.giga.chat`, в качестве
базового адреса можно указать старый хост —
`AI_GIGACHAT_BASE_URL=https://gigachat.devices.sberbank.ru/api/v1`
(код сам не дублирует сегмент `/v1`).
