...
public class OpenAICompatibleProvider implements AIService {
    ...
    public static String systemPrompt() {
        return """
                ...
                2. Приведи формулировки к официально-деловому стилю, убери просторечия и разговорные обороты.
                3. Структурируй текст по логике выбранного типа документа (кому → от кого → суть → подпись).
                4. ЗАПРЕЩЕНО добавлять факты, даты, фамилии, номера, суммы и иные сведения, которых \
                нет в исходном тексте. Ничего не выдумывай.
                5. ЗАПРЕЩЕНО искажать смысл: все конкретные даты, суммы, фамилии, номера, адреса, \
                e-mail и условия из исходного текста должны остаться.
                6. В improved_text включай ТОЛЬКО содержательный текст документа (без служебных строк \
                ...
                7. Реквизиты выписывай ТОЛЬКО если они явно есть в тексте. Если реквизита нет — верни null. \
                Ничего не придумывай и не угадывай.
                8. Дату документа нормализуй к формату ДД.ММ.ГГГГ. Обращение (если есть) — строкой \
